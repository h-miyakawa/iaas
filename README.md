...
## Webインターフェースの使い方
1. Webページへアクセス
	* ユーザのPCからWebブラウザで以下のURLを入力すると、IaaSログインページヘ移動できます。

		http://(ControllerのIPアドレス)/CodeIgniter/form_murata
	
2. ユーザの登録とログイン
	* IaaSを利用する際のIDとパスワードを設定するために、ログインページ下部の"新規登録"をクリック
	* UserCreateページへ移動し、UserIDとパスワードを登録。このとき、すでに登録されているIDで新規登録を行った場合、登録不可として他のIDによる登録を促します。
	* ユーザ登録が成功し、登録したIDでログインすると、アクション選択ページへ移動。
	* アクション選択ページには、当該ユーザーが現在設定しているVMとファイアーウォールの一覧を表示しており、IaaSの利用状況を確認することが出来ます。
	* 当ページで、IaaSを利用する際のアクションを選択出来るようになっており、このページを起点として、IaaSを利用します。アクション一覧は以下となっています。
...
3. VMの作成
	* アクション選択ページでvm_createを選択すると、VM\_Createページへ移動。
	* フォームに記された4項目を入力し、submitボタンをクリックすると、VMを作成できます。
4. VMの起動・停止
	* VMを作成した段階ではVMは停止中であり、当VMを利用する事できません。そのため、当VMを起動する必要があります。その際には、アクション選択ページでvm\_state\_changeアクションを選択します。
	* VM\_STATE\_CHANGEページでは、作成したVMの一覧から、各VMの状態(停止中・起動中・セットアップ中)を確認することが出来ます。そして、停止中のVMには起動ボタン、起動中のVMには停止ボタンが付随されています。停止しているVMを起動するために、起動ボタンをクリックすると、VMを起動することが出来ます。VMの停止についても同様。
5. VMの編集
	* アクション選択ページでvm\_modifyを選択すると、VM\_MODIFYページへ移動。
	* フォームに記された4項目を入力。VMを編集するためには、VMは停止中である必要があるので、VMの状態が停止中のものから既存のVMを選択します。
6. VMの削除
	* アクション選択ページでvm\_deleteを選択すると、VM\_DELETEページへ移動。
	* VM\_DELETEの際には、既存のVMのうち、VM\_IDのみを入力することで削除できます。ただし、VMを編集する場合と同様に、削除するVMは停止中である必要があります。
7. ファイアーウォールの設定に関する操作
	* アクション選択ページでfw\_control_addを選択すると、ファイアーウォールのルール追加ページへ移動。
	* フォームに記された5項目を入力。
	* ファイアーウォールの設定編集・削除の際も、これまでと同様に、与えられたフォームに従い、入力を行うことによって、ファイアーウォールの設定を行うことができます。
8. ユーザの削除
	* IaaSを今後使用しなくなるといった場合のために、ユーザ情報を削除することが出来ます。その際には、アクション選択ページでuser\_deleteを選択。
	* USER\_DELETEページでは、確認のために当該ユーザーが登録したパスワードを入力。そのパスワードが適切であった場合にユーザー情報が削除され、再度そのIDでログインすることはできません。
	* ユーザ削除を実行するためには、当ユーザが作成したVMがすべて削除されている必要があります。
...
7. VM が起動すると管理画面にて IP アドレスが表示されるので ssh でアクセス
